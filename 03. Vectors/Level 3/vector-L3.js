...
/*
  ...
  itself, but we value correctness and consistency as much as we do performance.  If any code attempts to access an array index larger than the length an error condition happens.  Most languages will return an error of some kind indicating such.  Javascript will just return 'undefined'. Using Javascript, we must check that index is within bounds but other languages will do that for you and raise an error if not.  AS AN ASIDE: the C/C++ language don't do range checking for you either.  Serious security problems happen that way (Google: buffer overflow exploits) but Javascript won't expose any security issues over it like some other languages do.

  One advantage of creating an abstraction over an array isn't just simulating a dynamically resizing array, but also we can add safety to an unsafe idea.  It's natural to account for the conditions that happen around the first item and ones after.  When thinking about corner cases and edge cases we might usually consider the conditions around the first item, last item, and anything between.

  To have any real confidence, we need to go beyond that and check for the cases when the structure
  is empty, or what happens if we attempt to access anything before the first valid position or after the last valid position.
  ...
  Warm-Up
    ...
    [x] Understand why storage and .toArray() might show different views of the array (see L2 bonus)
    [x] Declare a javascript array.  Log an index that is less than zero, or more than length.  What
          happens?  Why? // Undefined because..it's actually adding an undefined item to those index like a dynamic array??
