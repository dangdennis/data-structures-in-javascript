...
/*
  A vector is a dynamic array that adjusts size when a certain threshold is met when adding or removing items.  It is essentially a _collection_ that behaves somewhat like an array.  There are some some differences. Arrays always allocate enough memory to hold enough elements to fill the full capacity. Then allow random access anywhere within that range at any time.

  Collections are different.  They are not technically an array though they may or may not use one as a storage mechanism behind the scenes.  They allow you to append new items to the end, insert new items anywhere near items that have already been inserted, and allow random access anywhere within that range.

  Collections are dynamic arrays and go by different names and may have slightly different nuanced behaviors between them but they are essentially the same idea.
  ...
  The code in this file is a partially implemented vector.  Your objective is to implement the resize functionality.  When the vector length has reached capacity, it must double in size.  Complete the following tasks:

   1. [ ] .resize() function that expands the storage space
   2.  [ ] .add() function to expand when needed

   3. [ ] .insert() function to expand when needed
